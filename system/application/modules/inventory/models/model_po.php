...
class Model_po extends Model
{
	...
	function add_po($data)
	{
       $ins = array(
                ...
                'via_pemesanan'     => $data['via_pemesanan'],
                'active'            => '1'
        );
    ...
    function daftar_list_po($date)
	{
		$query = $this->db->query("SELECT a.no_po, a.tanggal_po, a.nama_customer, a.telepon, a.telepon_2, a.telepon_3, a.bayar_awal, a.id_cara_pembayaran, 
								  	a.jumlah_bayar, a.nama_penerima, a.via_pemesanan, a.tanggal_pesan, (SELECT COUNT(b.no_po) FROM tb_penjualan b WHERE b.no_po = a.no_po AND b.active='1') jumlah_po 
    ...
    {
        $query = $this->db->query("SELECT a.no_po, a.tanggal_po, a.nama_customer, a.telepon, a.telepon_2, a.telepon_3, a.bayar_awal, a.id_cara_pembayaran, a.jumlah_bayar, b.nama_cara_pembayaran, a.nama_penerima, a.via_pemesanan FROM tb_po a LEFT JOIN tb_cara_pembayaran b ON b.id_cara_pembayaran=a.id_cara_pembayaran WHERE a.no_po='".$data."'");
		$result = $query->result_array();
    ...
    {    
		$result = $this->db->query("update tb_po set tanggal_po='".$data['tanggal_po']."', tanggal_pesan='".$data['tanggal_pesan']."', nama_penerima='".$data['nama_penerima']."', via_pemesanan='".$data['via_pemesanan']."', nama_customer='".$data['nama_customer']."', telepon='".$data['telepon']."', telepon_2='".$data['telepon_2']."', telepon_3='".$data['telepon_3']."', jumlah_bayar='".$data['jumlah_bayar']."', bayar_awal='".$data['bayar_awal']."', id_cara_pembayaran='".$data['id_cara_pembayaran']."' where no_po='".$data['no_po']."';");
        //$result = $this->db->query("update tb_po set tanggal_po='".$data['tanggal_po']."', nama_penerima='".$data['nama_penerima']."', via_pemesanan='".$data['via_pemesanan']."', nama_customer='".$data['nama_customer']."', telepon='".$data['telepon']."', telepon_2='".$data['telepon_2']."', telepon_3='".$data['telepon_3']."', jumlah_bayar='".$data['jumlah_bayar']."', bayar_awal='".$data['bayar_awal']."', id_cara_pembayaran='".$data['id_cara_pembayaran']."' where no_po='".$data['no_po']."';");
    }
