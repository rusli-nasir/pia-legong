<?php
class Model_cara_pembayaran_bhn extends Model
{
	function Model_cara_pembayaran_bhn()
	{
		parent::Model();
	}
	...
	function generate_id_cara_pembayaran()
	{
	   ...
	   $query = $this->db->query("SELECT id_cara_pembayaran FROM tb_cara_pembayaran_bhn 
       ...
       if($hasil == 0)
       {
            $id_cara_pembayaran = 'CBYH'.$yearmonth.'0000001';
       }
       else
       {
            ...
            $id_cara_pembayaran = 'CBYH'.$yearmonth.substr($joinref,-7);
       }
	...
	{
		$result = $this->db->query("SELECT id_cara_pembayaran, nama_cara_pembayaran, keterangan FROM tb_cara_pembayaran_bhn WHERE active=1");
		return $result;
	}
	...
	function insert_cara_pembayaran($data)
	{
        ...
        $this->db->insert('tb_cara_pembayaran_bhn', $ins);
	...
	function update_cara_pembayaran($data)
	{
		...
		$this->db->where($prm);
		$this->db->update('tb_cara_pembayaran_bhn', $upd);
	...
	{
		$result = $this->db->query("SELECT id_cara_pembayaran, nama_cara_pembayaran, keterangan FROM tb_cara_pembayaran_bhn 
	...
	function summary_cara_bayar($data)
	{
		$result = $this->db->query("SELECT a.id_cara_pembayaran, a.nama_cara_pembayaran, 
								   	...
								   	FROM tb_cara_pembayaran_bhn a 
									WHERE a.active='1' AND a.id_cara_pembayaran <> 'CRBY2011030000002' ORDER BY a.nama_cara_pembayaran ASC;");
		return $result;
	}
}
?>